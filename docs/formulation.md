...
**Corollary (the guarded meet).** B4's pass-through is the right response when *nothing* can comply, and the wrong one when only the composition cannot. Where the mandate is itself satisfiable, enforce instead the greatest satisfiable lower bound of {θ_neg, θ_man} that is at or below θ_man; when the meet is empty that element is θ_man itself. The platform then never governs less than its own mandate, whatever the parties agree between themselves. Implemented as `guarded_meet` in `src/contract.py` and exercised in `tests/test_contract_algebra.py`; measured in `docs/notes/2026-09-08-guarded-meet-results.md`.

**Why this is not optional.** *(empirical, 2026-08-11 — `docs/notes/2026-08-10-arm-c-negotiated-contract.md` and `docs/notes/2026-08-11-arm-c-meet.md`)* A contract the parties negotiate between themselves **need not refine the mandate they are subject to**. On `bargain_3_9` **no self-negotiated envelope refined the platform's θ on any of the 9 pairs**: the inferred budget is looser on every one, by 1.05–1.52×, identically in every seed because both opening positions are deterministic given the scenario. The cause is structural — a seller's opening ask sits above the buyer's reservation — so the direction is categorical rather than an estimate with a spread. Enforcing the negotiated contract alone therefore licenses exactly what the mandate forbids, and arm C settles a higher fraction of breaching deals than doing nothing at all.

Composition is what makes a self-negotiated contract safe to *permit*: the platform lets the parties agree what they like and enforces θ_negotiated ∧ θ_mandate, which by Proposition 3 is precisely "everything both contracts allow, and nothing else". **Measured (arm C-meet, 5 seeds):** settled breaches against the mandate return to **0 of 15** and overspend to **£0.00** (arm C: 2 of 14, £0.48), while 15 deals still close against arm B's 16 — so the guarantee is recovered without being bought with closure. The enforced contract refines the mandate in 27 of 27 instances and the negotiated envelope in 27 of 27.

Two caveats belong with it. The meet can be **unsatisfiable when both operands are satisfiable** — observed once, when a buyer counter-offered above its own stated budget — and composition then correctly reports no common ground rather than filtering. And a **degenerate** meet (safe set a single point) defeats the QP projection, which needed a closed-form feasibility fallback; see §10 limitation B7.
